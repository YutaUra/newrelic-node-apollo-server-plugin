...
/**
 * @param {import('apollo-server-types').GraphQLRequestContextWillSendResponse<import('apollo-server-types').BaseContext>} responseContext
 * @returns {null | { operationName: string, operationType: string, deepestUniquePath: string[], cleanedQuery: string }}
 */
...
function getDetailsFromDocument(responseContext) {
  ...
  const pathAndArgs = getDeepestPathAndQueryArguments(
    responseContext.operation,
    responseContext.document
  )
  ...
  const definitionName = responseContext.operationName
 ...
 *
 * @param {import('graphql').SelectionNode} selection node in graphql document AST
 * @returns {selection is import('graphql').InlineFragmentNode}
 */
 ...
 * @param {import('graphql').OperationDefinitionNode} definition
 * @param {import('graphql').DocumentNode} document
 * @returns {{ deepestPath: string[], argLocations: import('graphql').Location[] }}
 */
function getDeepestPathAndQueryArguments(definition, document) {
  /** @type {string[]} */
  let deepestPath = []
  let foundDeepestPath = false
  /** @type {import('graphql').Location[]} */
  let argLocations = []
   ...
   * @param {import('graphql').SelectionNode} selection
   * @param {string[] | undefined} currentParts
   * @returns {void}
   */
  function searchSelection(selection, currentParts) {
    ...
    if (
      ...
      filtered
        .filter((innerSelection) => innerSelection.kind !== 'FragmentSpread')
        .forEach((innerSelection) => {
      ...
      filtered
        ...
        .forEach((innerSelection) => {
          const fragmentDefinition = document.definitions.find(
            (node) =>
              node.kind === 'FragmentDefinition' && node.name.value === innerSelection.name.value
          )
...
/**
 * @param {ReadonlyArray<import('graphql').SelectionNode>} selections
 * @returns {ReadonlyArray<import('graphql').SelectionNode>}
 */
function filterSelectionsForDeepestPath(selections) {
  return selections.filter((currentSelection) => {
    ...
    if (currentSelection.kind === 'InlineFragment' || currentSelection.kind === 'FragmentSpread') {
      return true
    }
